...
`forge-chain-mempool-lab` is centered on blockchain tooling work in C. The technical target is: Implement a C blockchain tooling project for mempool format conversion, using round-trip fixtures and lossless normalization checks.
...
The score is deterministic. The current threshold is 166. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 2.
